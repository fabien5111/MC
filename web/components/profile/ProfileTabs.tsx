...
export function ProfileTabs({
  ...
  return (
    <section className="mt-16">
        ...
        <div className="py-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {recipes.map((r) => {
            ...
            return (
              <div
                key={r.id}
                className="group bg-surface-container-lowest border border-outline-variant p-2 rounded-lg hover:shadow-lg transition-all duration-300"
              >
                ...
                .map((f) => {
                  ...
                  return (
                    <Link
                      key={r.id}
                      href={`/recette/${r.id}`}
                      className="group bg-surface-container-lowest border border-outline-variant p-2 rounded-lg hover:shadow-lg transition-all duration-300 block"
                    >
      ...
      {/* Messagerie (maquette statique, comme la version vanilla) */}
      {tab === 'messaging' && (
        <div className="py-10">
          <div className="flex items-center justify-center h-[300px] border border-outline-variant rounded-xl bg-white text-on-surface-variant">
            <div className="text-center">
              <span className="material-symbols-outlined text-[48px] mb-3 block">forum</span>
              <p className="font-body-md">La messagerie arrivera dans une prochaine phase.</p>
            </div>
          </div>
        </div>
      )}
